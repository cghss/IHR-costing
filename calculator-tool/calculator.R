...
library(tidyr) ## for the pivot_longer function to reshape data
...
## format all unit costs and all numeric scores as numeric variables
unit_costs$value <- as.numeric(as.character(unit_costs$value))
...
line_item_costs <- line_items %>%
  ...
  left_join(unit_costs[,which(names(unit_costs) %in% c("category_sloan", "category", "subcategory", "unit_cost", "unit", "value"))], by = "unit_cost") %>%
  ...
  mutate(y1cost = ifelse(costed_y1 == TRUE, value*custom_multiplier_1*custom_multiplier_2*administrative_multiplier_unit, 0)) %>%
  mutate(y2cost = ifelse(costed_y2 == TRUE, value*custom_multiplier_1*custom_multiplier_2*administrative_multiplier_unit, 0)) %>%
  mutate(y3cost = ifelse(costed_y3 == TRUE, value*custom_multiplier_1*custom_multiplier_2*administrative_multiplier_unit, 0)) %>%
  mutate(y4cost = ifelse(costed_y4 == TRUE, value*custom_multiplier_1*custom_multiplier_2*administrative_multiplier_unit, 0)) %>%
  mutate(y5cost = ifelse(costed_y5 == TRUE, value*custom_multiplier_1*custom_multiplier_2*administrative_multiplier_unit, 0)) %>%
  mutate(cost_5yrs = y1cost + y2cost + y3cost + y4cost + y5cost) %>%
  select(line_item_id, include, metric_score_id, pillar, capacity, indicator, score_numeric, score_text, attribute, requirement, action, description, unit_cost, administrative_level, y1cost, y2cost, y3cost, y4cost, y5cost, cost_5yrs, cost_type, value, custom_multiplier_1, custom_multiplier_2, administrative_multiplier_unit, relevant_references, optional_cost, notes_assumptions, category_sloan, category, subcategory)
...
yearly_costs %>%
  ...
  geom_bar(stat = "identity", color = "black", fill = "#134B70") +
  labs(x = "Year", y = "Cost", title = "Yearly costs (5 years)") +
  theme_minimal()
